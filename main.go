...
func main() {
	boshManifest := flag.String("manifest", "cf-deployment.yml", "BOSH manifest with releases to fetch")
	flag.Parse()

	boshfile := getBoshFile(*boshManifest)
...
func getBoshFile(manifest string) bosh_file.BoshFile {
	...
	if _, err := os.Stat("BOSHFILE"); err == nil {
		...
		boshfile, err = bosh_manifest.ParseFile(manifest)
	} else {
